function [ M, nCount] = HWexplicit_diffusion(x0,b,lambda,dt)
...
for col=1:nt-1
    ...
    M(1,col+1) = (M(1,col) + lambda*( M(2,col) -2*M(1,col)+ M(2,col)) + (term2 *(nCount(1,col))*(b+(v1*M(1,col)/(k1 +M(1,col))))) - (deltaT*(y*M(1,col)/(M(1,col)+ky))) );
    
    %For the interior points.
    for row=2:nx-1
        nCount(row, col+1) = nFoward(nCount(row,col),M(row,col),deltaT);
        M(row,col+1) = (M(row,col) + lambda*( M(row+1,col) -2*M(row,col)+ M(row-1,col)) + (term2 *(nCount(row,col))*(b+(v1*M(row,col)/(k1 +M(row,col)))) ) - (deltaT*(y*M(row,col)/(M(row,col)+ky))) );
    end
